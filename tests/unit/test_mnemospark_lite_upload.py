...
class PostUploadReliabilityTests(unittest.TestCase):
    def test_post_upload_sends_v1_paymentpayload_shape_to_cdp(self):
        event = {
            "body": json.dumps(
                {
                    "filename": "artifact.bin",
        ...
        self.assertEqual(payload_sent.get("x402Version"), 2)
        self.assertEqual(payload_sent.get("scheme"), "exact")
        self.assertEqual(payload_sent.get("network"), "base")
        self.assertIn("payload", payload_sent)
